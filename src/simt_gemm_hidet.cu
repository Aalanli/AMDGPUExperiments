...
struct SimtGemmCTile {

};

template <int nthreads, int OuterM, int MidM, int InnerM>
struct SimtGemmATile {
    static constexpr int block_m = OuterM * MidM * InnerM;
    static constexpr int ldg = block_m / (nthreads / block_k);
    float load[ldg];
    float regs[OuterM][InnerM];
...
extern "C" __attribute__((visibility("default"))) bool LAUNCH_NAME(float* a, float* b, float* c, int m, int k, int n, int version) {
    cudaDeviceProp prop;
    ...
    if (version == 0)
        simt_gemm_hidet_kernel<<<grid, block>>>(a, b, c, m, n, k);
    else if (version == 1)
        simt_gemm_hidet_kernelv2<<<grid, block>>>(a, b, c, m, n, k);
    else if (version == 2)
        simt_gemm_hidet_kernelv3<<<grid, block>>>(a, b, c, m, n, k);
    else if (version == 3)
        simt_gemm_hidet_kernelv4<<<grid, block>>>(a, b, c, m, n, k);
    else
        return false;
